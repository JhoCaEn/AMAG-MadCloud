module.exports = class ReplicationMasterdataFuelTypeService extends cds.ApplicationService {
    async init() {

        const LOG = cds.log('replication-masterdata-fueltype')

        const db = await cds.connect.to('db')
        const client = await require('../connect')()

        const { FuelTypes } = db.entities('retail.dwb')
        const { NotFoundError } = require('../errors')
        const brands = require('../brands')

        this.on('replication-masterdata:fueltype/changed', async ({ data: { BRAND, TYPE } = {} } = {}) => replicate(BRAND, TYPE))
        this.on('replication-masterdata:fueltype/deleted', async ({ data: { BRAND, TYPE } = {} } = {}) => replicate(BRAND, TYPE))
        ...
        const save = async (data) => {
            ...
            const exists = await db.exists(FuelTypes, { brand_code, id }).forUpdate()

            return exists
                ? db.update(FuelTypes, { brand_code, id }).set(data)
                : db.create(FuelTypes, data)
        }
        ...
        const remove = async (brand_code, id) => {
            ...
            if (exists)
                return db.delete(FuelTypes, { brand_code, id })
        }
