module.exports = class ReplicationMasterdataModelSeriesService extends cds.ApplicationService {
    async init() {

        const LOG = cds.log('replication-masterdata-modelseries')

        const db = await cds.connect.to('db')
        const client = await require('../connect')()

        const { ModelSeries } = db.entities('retail.dwb')
        const { NotFoundError } = require('../errors')
        const brands = require('../brands')

        this.on('replication-masterdata:modelseries/changed', async ({ data: { BRAND, MODELSERIES } = {} } = {}) => replicate(BRAND, MODELSERIES))
        this.on('replication-masterdata:modelseries/deleted', async ({ data: { BRAND, MODELSERIES } = {} } = {}) => replicate(BRAND, MODELSERIES))
        ...
        const save = async (data) => {
            ...
            const exists = await db.exists(ModelSeries, { brand_code, id }).forUpdate()

            return exists
                ? db.update(ModelSeries, { brand_code, id }).set(data)
                : db.create(ModelSeries, data)
        }
        ...
        const remove = async (brand_code, id) => {
            ...
            if (exists)
                return db.delete(ModelSeries, { brand_code, id })
        }
