module.exports = class ReplicationMasterdataEquipmentCategoryService extends cds.ApplicationService {
    async init() {

        const LOG = cds.log('replication-masterdata-equipmentcategory')

        const db = await cds.connect.to('db')
        const client = await require('../connect')()

        const { EquipmentCategories } = db.entities('retail.dwb')
        const { NotFoundError } = require('../errors')
        const brands = require('../brands')

        this.on('replication-masterdata:equipmentcategory/changed', async ({ data: { BRAND, CATEGORY } = {} } = {}) => replicate(BRAND, CATEGORY))
        this.on('replication-masterdata:equipmentcategory/deleted', async ({ data: { BRAND, CATEGORY } = {} } = {}) => replicate(BRAND, CATEGORY))
        ...
        const get = async (brand, id) => {
            return client.getEquipmentCategory({
                id,
                brand,
                $expand: '_Texts'
            })
        ...
        const save = async (data) => {
            ...
            const exists = await db.exists(EquipmentCategories, { brand_code, id }).forUpdate()

            return exists
                ? db.update(EquipmentCategories, { brand_code, id }).set(data)
                : db.create(EquipmentCategories, data)
        }
        ...
        const remove = async (brand_code, id) => {
            ...
            if (exists)
                return db.delete(EquipmentCategories, { brand_code, id })
        }
