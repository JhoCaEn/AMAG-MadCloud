module.exports = class ReplicationMasterdataTransmissionTypeService extends cds.ApplicationService {
    async init() {

        const LOG = cds.log('replication-masterdata-transmissiontype')

        const db = await cds.connect.to('db')
        const client = await require('../connect')()

        const { TransmissionTypes } = db.entities('retail.dwb')
        const { NotFoundError } = require('../errors')
        const brands = require('../brands')

        this.on('replication-masterdata:transmissiontype/changed', async ({ data: { BRAND, TYPE } = {} } = {}) => replicate(BRAND, TYPE))
        this.on('replication-masterdata:transmissiontype/deleted', async ({ data: { BRAND, TYPE } = {} } = {}) => replicate(BRAND, TYPE))
        ...
        const get = async (brand, id) => {
            return client.getTransmissionType({
                id,
                brand,
                $expand: '_Texts'
            })
        ...
        const save = async (data) => {
            ...
            const exists = await db.exists(TransmissionTypes, { brand_code, id }).forUpdate()

            return exists
                ? db.update(TransmissionTypes, { brand_code, id }).set(data)
                : db.create(TransmissionTypes, data)
        }
        ...
        const remove = async (brand_code, id) => {
            ...
            if (exists)
                return db.delete(TransmissionTypes, { brand_code, id })
        }
