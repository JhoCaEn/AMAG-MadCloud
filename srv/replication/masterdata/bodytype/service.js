module.exports = class ReplicationMasterdataBodyTypeService extends cds.ApplicationService {
    async init() {

        const LOG = cds.log('replication-masterdata-bodytype')

        const db = await cds.connect.to('db')
        const client = await require('../connect')()

        const { BodyTypes } = db.entities('retail.dwb')
        const { NotFoundError } = require('../errors')
        const brands = require('../brands')

        this.on('replication-masterdata:bodytype/changed', async ({ data: { BRAND, TYPE } = {} } = {}) => replicate(BRAND, TYPE))
        this.on('replication-masterdata:bodytype/deleted', async ({ data: { BRAND, TYPE } = {} } = {}) => replicate(BRAND, TYPE))
        ...
        const save = async (data) => {
            ...
            const exists = await db.exists(BodyTypes, { brand_code, id }).forUpdate()

            return exists
                ? db.update(BodyTypes, { brand_code, id }).set(data)
                : db.create(BodyTypes, data)
        }
        ...
        const remove = async (brand_code, id) => {
            ...
            if (exists)
                return db.delete(BodyTypes, { brand_code, id })
        }
